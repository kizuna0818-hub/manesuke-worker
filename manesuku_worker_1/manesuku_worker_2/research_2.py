...
_MK = {
 "us": ("({asset} は米国株・ETF・暗号資産(BTC等)。価格はUSD建て)",
        "株式/ETFは split/配当 調整後のUSD、暗号資産はUSD現物"),
 "jp": ("({asset} は日本株(東証。例 7203=トヨタ, 6758=ソニー, 285A=Kioxia)。価格は必ず日本円(JPY)建て)",
        "東証の終値ベース、split調整後の日本円(JPY)"),
}
...
def research_single_stock(asset, entry_date, market="us", api_key=None, model="claude-sonnet-4-6"):
    api_key = api_key or os.environ.get("ANTHROPIC_KEY")
    if not api_key:
        raise ValueError("needsReview: ANTHROPIC_KEY 未設定")
    note, unit = _MK.get(str(market).lower(), _MK["us"])
    prompt = PROMPT.format(asset=asset, entry=entry_date,
                           market_note=note.format(asset=asset), unit=unit)
    body = {
