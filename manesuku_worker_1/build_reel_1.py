...
CSS = r'''
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .barsbox{display:flex;gap:calc(var(--u)*9);align-items:flex-end;justify-content:center;height:calc(var(--u)*40);margin-top:calc(var(--u)*7)}
  .bcol{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%}
  .bval{font-family:var(--mono);font-weight:700;font-size:calc(var(--u)*3.5);color:var(--cream);margin-bottom:calc(var(--u)*1.6);white-space:nowrap}
  .brect{width:calc(var(--u)*22);height:0;border-radius:calc(var(--u)*1.6) calc(var(--u)*1.6) 0 0}
  .blabel{font-size:calc(var(--u)*2.6);color:var(--cream-dim);margin-top:calc(var(--u)*2.6);font-weight:700}
...
def beat_html(b, i, charts):
    ...
    if t == "bars":
        cur = b.get("currency","yen"); labels = b.get("labels",["投資額","利益"])
        vals = [b["invested"], b["profit"]]
        cols = ""
        for j,(val,lab) in enumerate(zip(vals, labels)):
            bg = "linear-gradient(180deg,#E7C77A,#C8993C)" if j==1 else "#3a352b"
            cols += (f'<div class="bcol"><div class="bval" id="bv{i}_{j}">{fmt(val,cur)}</div>'
                     f'<div class="brect" id="br{i}_{j}" style="background:{bg}"></div>'
                     f'<div class="blabel">{lab}</div></div>')
        return bid, (f'<div class="beat bars" id="{bid}">'
                     f'<div class="stat" style="font-size:calc(var(--u)*13)">{b["mult"]}</div>'
                     f'<div class="barsbox">{cols}</div></div>')
    if t == "lesson":
...
def build(spec):
    ...
    beats=[]; t0=0.0; charts={}; bars={}
    for i,b in enumerate(spec["beats"]):
        dur=float(b["dur"]); s=t0; e=t0+dur; t0=e; beats.append((i,b,s,e))
        if b["type"]=="bars":
            inv=float(b["invested"]); pf=float(b["profit"])
            grow=float(b.get("grow", min(dur-1.0, 2.0)))
            bars[str(i)]={"start":round(s+0.4,2),"grow":round(grow,2),"vals":[inv,pf],
                          "max":max(inv,pf,1.0),"maxH":30,"cur":b.get("currency","yen")}
        if b["type"]=="chart":
...
window.renderAt=function(t){{
  ...
  for(const bk in CFG.bars){{const bc=CFG.bars[bk];const p=t>=bc.start?Math.min(1,(t-bc.start)/bc.grow):0;
    for(let j=0;j<bc.vals.length;j++){{
      el('br'+bk+'_'+j).style.height='calc(var(--u)*'+(Math.max(0,bc.vals[j])/bc.max*bc.maxH*p).toFixed(2)+')';
      el('bv'+bk+'_'+j).textContent=FMT(bc.vals[j]*p,bc.cur);}}}}
}};
